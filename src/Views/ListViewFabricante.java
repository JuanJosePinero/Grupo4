				...
				if (fila_seleccionada >= 0) {
					...
					if (decision == 0) {
						try {
							...
							JOptionPane.showMessageDialog(ListViewFabricante.this,
	...
	private void showFabricante() {
		try {
			...
			for (int i = 0; i < this.fabricante.size(); i++) {
				dtm.addRow(new Object[] { this.fabricante.get(i).getIdFabricante(), this.fabricante.get(i).getNombre(),
						this.fabricante.get(i).getPais() });
			}
