...
public class ClientService {
	 ...
	 public void save(Connection conexion, Cliente cliente) throws SQLException{
	      try{
	         ...
	         if(cliente.getIdClientes() == null){
	            consulta = conexion.prepareStatement("INSERT INTO " + this.tabla + "(nombre, direccion,rol,usuario,contraseya) VALUES(?, ?, ?, ?, ?)");
	            ...
	            consulta = conexion.prepareStatement("UPDATE " + this.tabla + " SET nombre = ?, direccion = ?,rol = ?, usuario = ?, contraseya = ? WHERE id = ?");
	            consulta.setString(1,cliente.getNombre());
	            consulta.setString(2, cliente.getDireccion());
	            consulta.setString(3, cliente.getRol());
	            consulta.setString(4, cliente.getNombreUsuario());
	 ...
	 public Cliente getCliente(Connection conexion, int id) throws SQLException {
	      ...
	      try{
	         PreparedStatement consulta = conexion.prepareStatement("SELECT nombre, direccion,rol,usuario,contraseya "
	 ...
	 public List<Cliente> getCliente(Connection conexion) throws SQLException{
	      ...
	      try{
	         PreparedStatement consulta = conexion.prepareStatement("SELECT id,nombre,direccion ,rol,usuario,contraseya"
	                 + " FROM " + this.tabla);
	         ResultSet resultado = consulta.executeQuery();
	         while(resultado.next()){
