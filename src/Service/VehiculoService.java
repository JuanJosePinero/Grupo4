...
public class VehiculoService {
	   ...
	   public void save(Connection conexion, Vehiculo vehiculo) throws SQLException{
	      try{
	         ...
	         if(vehiculo.getIdVehiculos() == null){
	            consulta = conexion.prepareStatement("INSERT INTO " + this.tabla + "(modelo, marca, anyo, color, precio, idFabricante, ruta, numcomentarios,numvaloraciones) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)");
	            ...
	            consulta = conexion.prepareStatement("UPDATE " + this.tabla + " SET modelo = ?, marca = ?, anyo = ?, color = ?, precio = ?, idFabricante = ?, ruta = ?, comprado = ?, alquilado = ?, numcomentarios = ?, numvaloraciones = ?  WHERE idVehiculos = ?");
	            consulta.setString(1, vehiculo.getModelo());
	            consulta.setString(2, vehiculo.getMarca());
	            consulta.setInt(3, vehiculo.getAnyo());     
	            consulta.setString(4, vehiculo.getColor());
	   ...
	   public Vehiculo getVehiculo(Connection conexion, int id) throws SQLException {
	      ...
	      try{
	         PreparedStatement consulta = conexion.prepareStatement("SELECT idVehiculos, modelo, marca, anyo, color, precio, idFabricante, ruta, comprado, alquilado,numcomentarios,numvaloraciones"
	         ...
	         while(resultado.next()){
	        	 vehiculo = new Vehiculo(resultado.getInt("idVehiculos"), resultado.getString("modelo"), resultado.getString("marca"), 
	                    resultado.getInt("anyo"), resultado.getString("color"), resultado.getFloat("precio"), resultado.getInt("idFabricante"), resultado.getString("ruta"), resultado.getInt("comprado"),
	                    resultado.getInt("alquilado"),resultado.getInt("numcomentarios"),resultado.getInt("numvaloraciones"));
	         }
	      }catch(SQLException ex){
	         throw new SQLException(ex);
	      }
	   ...
	   public List<Vehiculo> getAllVehiculosCliente(Connection conexion) throws SQLException {
		    ...
		    try {
		        PreparedStatement consulta = conexion.prepareStatement("SELECT v.idVenta, v.fechaHora, ve.idVehiculos, ve.Modelo, ve.Marca, ve.Anyo, ve.Color, ve.Precio, ve.idFabricante "
		                ...
		                + "WHERE v.idVehiculo = ve.idVehiculos "
		                + "AND v.idCliente = ?;");
